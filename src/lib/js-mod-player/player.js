...
export class ModPlayer {
    ...
    async load(url, volume, enableSubscriptionsByDefault) {
        if (this[WORKLET]) this.unload();

        this.mod = await loadMod(url);
        this.totalRows = this.mod.patternTable.reduce((prev, curr) => {
            return prev + this.mod.patterns[curr].rows.length
        }, 0)
        if (!this[AUDIO]) this[AUDIO] = new AudioContext({sampleRate: 44100});
        this[GAIN] = this[AUDIO].createGain();
        this[GAIN].gain.value = volume ? volume : 0.3;
        ...
        if (enableSubscriptionsByDefault) {
    ...
    async loadBuffer(buffer, volume, enableSubscriptionsByDefault) {
        if (this[WORKLET]) this.unload();

        this.mod = new Mod(buffer);
        this.totalRows = this.mod.patternTable.reduce((prev, curr) => {
            return prev + this.mod.patterns[curr].rows.length
        }, 0)
