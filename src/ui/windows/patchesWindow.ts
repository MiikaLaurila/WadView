...
const getTexture = (texture: WadTexture) => {
	...
	const imageData = ctx.createImageData(texture.width * 3, texture.height * 3, {
		colorSpace: "srgb",
	});

	for (const texPatch of texture.patches) {
		const patchName = textures.patchNames[texPatch.patchNum];
		const patch = textures.patches[patchName];
		if (patch)
			writePatchToTextureImageData(patch, imageData, texture, texPatch);
	}
...
export const initPatchesWindowModule = () => {
	...
	const sortedPatches = [...textures.patchNames]
		.map((n, i) => {
			return { name: n, idx: i };
		})
		.sort((a, b) => a.name.localeCompare(b.name));

	for (const pname of sortedPatches) {
		...
		const textures1WithPname = textures.texture1.filter((t) =>
			t.patches.find((p) => p.patchNum === pname.idx),
		);
		const textures2WithPname = textures.texture2.filter((t) =>
			t.patches.find((p) => p.patchNum === pname.idx),
		);
		...
		for (const entry of textures1WithPname) {
		...
		for (const entry of textures2WithPname) {
			const row = document.createElement("div");
			row.appendChild(getTextureCell(entry.name, entry));
