...
import type {
	DSDAResponse,
	DSDAResponseList,
} from "../../interfaces/DSDAResponse";
import type {
	IdGamesResponse,
	IdGamesResponseFile,
} from "../../interfaces/IdGamesResponse";
...
const executeIdGamesSearch = (val: string, loadParent: HTMLElement) => {
	...
	fetch(
		`${corsProxy + idGamesApiUrl}action=search&query=${urlFormatted}&type=title&sort=date&out=json`,
	)
		.then(async (res) => {
...
const executeDsdaSearch = (val: string, loadParent: HTMLElement) => {
	const parseWadPage = (doc: Document): DSDAResponse | null => {
		...
		const [linkBar] = Array.from(
			doc.getElementsByClassName("p-short one-line"),
		);
		if (head && linkBar) {
	...
	fetch(
		`${corsProxy + dsdaBaseUrl}/search?utf8=%E2%9C%93&search=${urlFormatted}&commit=Search`,
	)
		.then(async (res) => {
			...
			if (!isMultiResult) {
				...
				const promises = linksToPages.map((l) =>
					fetch(corsProxy + dsdaBaseUrl + l).then(async (r) =>
						parseWadPage(
							domParser.parseFromString(await r.text(), "text/html"),
						),
					),
				);
				const responses = await Promise.all(promises);
				lastDsdaGameResult = responses.filter(
					(r) => r !== null,
				) as DSDAResponseList;
			}
			writeResults("dsda");
...
const createSearchDiv = (
	host: string,
	onSearch: (val: string, loadParent: HTMLElement) => void,
) => {
...
const writeResults = (type: "idgames" | "dsda") => {
	...
	if (type === "idgames") {
		...
		for (const f of lastDsdaGameResult) {
...
const createIdgamesWadCard = (f: IdGamesResponseFile) => {
	...
	useAsWadButton.onclick = () => {
		const urlBase =
			idGamesMirrorList[Math.floor(Math.random() * idGamesMirrorList.length)];
