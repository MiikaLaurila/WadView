...
window.addEventListener("resize", () => {
	if (app && viewport && dim) {
		...
		setDimensions();
		app.renderer.resize(
			dim.width + canvasPadding * 2,
			dim.height + canvasPadding * 2,
		);
		reDrawMap();
	}
});
...
const setDimensions = (): void => {
	if (!bounds) {
		console.error(
			"Can't set dimensions of map because there are no map bounds set",
		);
		return;
	}
...
const initializeMap = () => {
	...
	if (!bounds || !dim) {
		console.error(
			"Can't initialize map because dimensions or bounds are missing",
		);
		return;
	}
	...
	app = new Application({
		...
		backgroundColor: utils.string2hex(
			selectedPalette.getBackground(selectedPalette),
		),
...
const drawLines = (graphy: Graphics) => {
	...
	if (lineCache && Object.keys(lineCache).length > 0) {
		for (const key of Object.keys(lineCache)) {
			const cachedLine = lineCache?.[key];
			if (!cachedLine) continue;
			for (const line of cachedLine) {
				graphy.lineStyle({
					color: utils.string2hex(key),
					width: graphy.line.width,
				});
				graphy.drawPolygon([line.vStart, line.vEnd]);
			}
		}
	} else {
		const sortedLineDefs = mapData.linedefs.sort((a, b) => {
		...
		for (const line of sortedLineDefs) {
			...
			const vStart = transformMapPointToCanvas({
				...mapData.vertices[line.start],
			});
			const vEnd = transformMapPointToCanvas({
				...mapData.vertices[line.end],
			});
			const color = selectedPalette.getLineColor(
				selectedPalette,
				line,
				mapData,
			);
...
const thingIsRenderable = (t: WadMapThingDehacked): boolean => {
	if (!enabledThingGroups.includes(t.thingGroup as WadMapThingGroup))
		return false;
	if (showMultiPlayer === 1 && t.flagsString.includes("NET_ONLY")) return false;
	if (showMultiPlayer === 2 && !t.flagsString.includes("NET_ONLY"))
		return false;
	if (showDifficulty === 1 && !t.flagsString.includes("ON_SKILL_EASY"))
		return false;
	if (showDifficulty === 2 && !t.flagsString.includes("ON_SKILL_MEDIUM"))
		return false;
	if (showDifficulty === 3 && !t.flagsString.includes("ON_SKILL_HARD"))
		return false;
	if (hideDifficulty === 1 && t.flagsString.includes("ON_SKILL_EASY"))
		return false;
	if (hideDifficulty === 2 && t.flagsString.includes("ON_SKILL_MEDIUM"))
		return false;
	if (hideDifficulty === 3 && t.flagsString.includes("ON_SKILL_HARD"))
		return false;
	return true;
};
...
const drawThings = (graphy: Graphics) => {
	if (otherThingCache && monsterThingCache) {
		for (const thing of otherThingCache) {
			const dotSize = getDotSize(thing.size * 2);
			const x = thing.pos.x;
			const y = thing.pos.y;
			const ogLineW = graphy.line.width;
			graphy.lineStyle({
				color: utils.string2hex(thing.color),
				alignment: 1,
				width: 0,
			});
		...
		for (const thing of monsterThingCache) {
		...
		const sortedThings = applyDehacked(mapData.things)
			.filter((t) => thingIsRenderable(t))
		...
		for (const thing of sortedThings) {
			...
			if (!isMonster) {
				...
				graphy.lineStyle({
					color: utils.string2hex(color),
					alignment: 1,
					width: graphy.line.width * 0,
				});
				graphy.beginFill(utils.string2hex(color));
				graphy.drawRect(x - dotSize / 2, y - dotSize / 2, dotSize, dotSize);
				graphy.endFill();
				graphy.lineStyle({ width: ogLineW });
				otherThingCache.push({
					pos: { x, y },
					color,
					isDot: isMonster,
					size: thing.size,
...
const drawBlockmap = (graphy: Graphics): void => {
	...
	const blockMap = JSON.parse(
		JSON.stringify(mapData.blockMap),
	) as WadMapBlockMap;
	const gridSize = 128;
	...
	const ogLineW = graphy.line.width;
	graphy.lineStyle({
		color: utils.string2hex(gridColor),
		alignment: 1,
		width: graphy.line.width * 0.5,
	});
...
const drawGrid = (graphy: Graphics): void => {
	...
	const gridSize = 32;
	const [gridColor32, gridColor64] =
		selectedPalette.getGridColor(selectedPalette);
	...
	for (let i = 0; i < rowCount + 1; i++) {
		const is64 = i % 2 === 0;
		graphy.lineStyle({
			color: utils.string2hex(is64 ? gridColor64 : gridColor32),
			width: ogLineW * 0.3,
		});
		const yStart = transformMapYToCanvas(y0Orig + i * gridSize);
		graphy.drawPolygon([
...
const drawHover = (data: HoverData) => {
	...
	data.things.forEach((t, idx) => {
		...
		hoverDiv.appendChild(
			getHoverRow("TYPE: ", `${t.thingType} | ${t.thingTypeString}`),
		);
...
const getButtonArea = (mapName: string) => {
	...
	maxRes.oninput = (e: Event) => {
		...
		const nan = Number.isNaN(Number(target.value));
		if (!nan) {
	...
	for (const palette of mapPalettes) {
...
const getToggleArea = (mapName: string) => {
	...
	const generateThingToggleButton = (
		toggle: WadMapThingGroup | "ALL" | "DECO",
	) => {
		...
		container.onclick = () => {
			if (toggle === "ALL") {
				...
				removeOrAddToggle([
					WadMapThingGroup.OBSTACLE,
					WadMapThingGroup.DECORATION,
				]);
			} else {
	...
	thingButtonContainer.appendChild(
		generateThingToggleButton(WadMapThingGroup.MONSTER),
	);
	thingButtonContainer.appendChild(
		generateThingToggleButton(WadMapThingGroup.ARTIFACT),
	);
	thingButtonContainer.appendChild(
		generateThingToggleButton(WadMapThingGroup.POWERUP),
	);
	thingButtonContainer.appendChild(
		generateThingToggleButton(WadMapThingGroup.WEAPON),
	);
	thingButtonContainer.appendChild(
		generateThingToggleButton(WadMapThingGroup.AMMO),
	);
	thingButtonContainer.appendChild(
		generateThingToggleButton(WadMapThingGroup.KEY),
	);
	thingButtonContainer.appendChild(
		generateThingToggleButton(WadMapThingGroup.OTHER),
	);
	...
	const generateNetToggleButton = () => {
		...
		const getImg = () => {
			...
			if (showMultiPlayer === 2) return 'url("only.png")';
			return "";
		};
	...
	const generateDifficultyButton = (dir: "hide" | "show") => {
		...
		const getImg = () => {
			...
			if (dir === "show" && diff === 0) return 'url("all.png")';
			if (diff === 1) return 'url("easy.png")';
			if (diff === 2) return 'url("medium.png")';
			if (diff === 3) return 'url("hard.png")';
			return "";
		};
		square.style.backgroundImage = getImg();
		container.appendChild(square);
	...
	flagAreaContainer.appendChild(
		// biome-ignore lint/suspicious/noAssignInExpressions: <explanation>
		generateBoolButton("Grid", () => (showGrid = !showGrid), showGrid),
	);
	flagAreaContainer.appendChild(
		generateBoolButton(
			"Automap",
